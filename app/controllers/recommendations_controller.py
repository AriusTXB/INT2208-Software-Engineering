...
@router.get("/{news_id}")
async def get_news(news_id: str, news_collection=Depends(get_article_collection), user_collection=Depends(get_user_collection)):
    return await RCMNewsSystem(news_collection, user_collection).get_news(news_id)

@router.get("/view/{news_id}")
async def view_news(news_: str, news_collection=Depends(get_article_collection), user_collection=Depends(get_user_collection)):
